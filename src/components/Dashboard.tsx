...
const Dashboard = () => {
  ...
  return (
    <div className="min-h-screen flex">
      ...
      <div className="w-64 bg-slate-800 text-white p-6">
        <div className="mb-8">
          <h2 className="text-xl font-bold mb-2">{user?.name?.toUpperCase() || "Guest"}</h2>
          <div className="h-0.5 bg-white/20"></div>
        </div>
        <nav className="space-y-4">
          <button
          ...
          <button
            onClick={() => setActiveEventsModalOpen(true)}
            className="flex items-center space-x-3 w-full p-3 rounded hover:bg-slate-700 transition"
          >
            <Clock className="w-5 h-5" />
            <span>Active Events</span>
          ...
          <button
            onClick={() => logout()}
            className="flex items-center space-x-3 w-full p-3 bg-red-600 rounded hover:bg-red-700 transition mt-8"
          >
            <LogOut className="w-5 h-5" />
            <span>LOGOUT</span>
          </button>
